$(document).ready(function() {
	...
	$('body').on('change', '#game-week', function() {
		...
		$('body').on('click', '.player-type-ctr > label:not(:first-child)', displayPosition);
	});
...
function runTests(args) {
	if('undefined' === typeof args) args = 'runAll';
	...
	if('runAll' === args) {
		...
		function ret(players) {
			trendScores = {};

			$.each(players, function(name, stats) {
				trendScores[name] = calcTrend(stats);
			});
...
function calcTrend(stats) {
	weeks = $.each(stats, function(k, v) {
		var kInt = parseInt(k),
			kNative = k;
		if(NaN === parseInt(kInt)) {
			// This is the trend property
		}
		else {
			// This is not the trend property
		}
	});
}

function displayPosition(e) {
	var playerType = $(e.target).attr('data-position'),
		fn, playerLetter, week;

	$(e.target).toggleClass('active');

	if($('.player-type-ctr > label.active').length === 0) {
		$(e.target).toggleClass('active');
		return; // End the function here and do not adjust what player type is displayed
	}

	$('.player-type-ctr > label:not(:first-child)').each(function(i, el) {
		playerLetter = $(el).attr('data-position').slice(0, 1);

		fn = $(el).hasClass('active') ? 'show' : 'hide';

		$('article.suggestions > .' + playerLetter)[fn]();
	});

	removeAllActiveSuggestions();

	$('article.suggestions > div').addClass('active-suggestions-' + $('.player-type-ctr > label.active').length);

	week = $('#game-week').val().split('').map(function(char, i) {
...
function removeAllActiveSuggestions() {
	var elClass,
		newClass;

	$('article.suggestions > div').each(function(i, el) {
		elClass = $(el).attr('class');

		if('undefined' !== elClass) {
			newClass = '';

			elClass.split(' ').forEach(function(cl) {
				if(-1 === cl.indexOf('active-suggestions-')) newClass += (' ' + cl);
			});

			$(el).attr('class', newClass);
		}
		else console.log('found no class for', el);
	});
}
...
function appendTrends(weeks) {
	var orderedPlayers = orderByPlayers(weeks),
		players = addTrends(orderedPlayers);

	console.log(players);
}
function orderByPlayers(weeks) {
	var players = {},
		plKey;

	$.each(weeks, function(num, week) {
		$.each(week, function(position, list) {
			if('undefined' === typeof(players[position])) players[position] = {};

			list.forEach(function(pl) {
				plKey = pl.name + '_' + pl.team;

				if('undefined' === typeof(players[position][plKey])) players[position][plKey] = {};

				players[position][plKey][num] = pl;
			});
		});
	});
...
function addTrends(players) {
	var curPl, ts;

	$.each(players, function(pos, plWeek) {
		$.each(plWeek, function(plKey, pl) {
			curPl = {
				start: {
					weekNum: null,
					points: 0
				...
				weeksPlayed: calcNumWeeks(pl)
			};

			$.each(pl, function(weekNum, stats) {
				if(weekNum < curPl.start.weekNum || null === curPl.start.weekNum) {
					curPl.start.weekNum = parseInt(weekNum);
					curPl.start.points = parseInt(stats.points);
				}
				if(weekNum > curPl.end.weekNum || null === curPl.end.weekNum) {
					curPl.end.weekNum = parseInt(weekNum);
					curPl.end.points = parseInt(stats.points);
				}
			});

			ts = {
				weeks: curPl.weeksPlayed,
				score: (curPl.end.points - curPl.start.points) / curPl.weeksPlayed,
				s: curPl.start.points,
				e: curPl.end.points
			};

			curPl.trendScore = ts;
			players[pos][plKey]['trend'] = curPl;
		});
	});

	return players;
}
function getTrend(startWeek, numWeeks, weighting) {
	var rootDir = 'weeklyscores/',
		defs = {
			startWeek: 'Week05.csv',
			numWeeks: 5,
			weighting: []
		},
		getData = {
			startWeek: 'undefined' === typeof startWeek ? defs.startWeek : startWeek,
			numWeeks: 'undefined' === typeof numWeeks ? defs.numWeeks : numWeeks,
			weighting: 'undefined' === typeof weighting ? defs.weighting : weighting
		},
		weeks = [],
		trends;
	...
	$.ajax('get_weeks.php', {
		...
		success: function(response) {
			...
			$.each(response, function(i, wp) {
				plWeeks[i] = formatPlayers(wp);
			});

			appendTrends(plWeeks);
		},
